...
Channel.prototype._startFade = function( step, limit ){
	var self = this;
	this.stopFade();
	this._fading = setInterval( function(){ self._fade(  0.1 , 1 ); }, self.crossTime / 10 );
	return this; 
}
...
function MusicPlayer( baseElement ){
	...
	channel1.on( 'musicEnded', function(){ if( self.currentChannel == self.channels[0] ) self.nextMusic();} );
	...
	channel2.on( 'musicEnded', function(){ if( self.currentChannel == self.channels[1] ) self.nextMusic();} );
...
MusicPlayer.prototype.init = function( list ){
	//console.log( list, this );
	this.resetList( list );
	this.nextMusic();
}

MusicPlayer.prototype.resetList = function( list ){
	if(list) this.list = list;
	this.pos = 0;
}

MusicPlayer.prototype.nextMusic = function(force){
	//if(!force)return;
	if( this.list.length == 0 ){ return; }
	if( this.list.length == this.pos ){ this.resetList() };
	this.currentMusic = this.list[ this.pos ];
	this.pos++;

	if( this.currentChannel ){ this.currentChannel.fadeDown(); }
