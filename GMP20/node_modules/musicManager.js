...
MusicManager.prototype.init = function(player_opts, server_options, db){
	...
	this.typeChoose = "normal";
	this.db = db;
	this.db.transaction(function (tx) {
		tx.executeSql('DROP TABLE musicas_temp;', [], null, self.SQLError);
		tx.executeSql('CREATE TABLE IF NOT EXISTS musicas_temp (audioid, name, artist, band, album, mood_velocity NUMERIC, mood_happy NUMERIC, path, playlist, type, date_start NUMERIC, date_end NUMERIC, hour_start NUMERIC, hour_end NUMERIC, weekdays, min_interval)',[],function(){
...
MusicManager.prototype.updateLocalList = function(callback){ //Faz a sincronização da lista local com a lista do servidor.
	...
	this.db.transaction(function (tx) {
		tx.executeSql('DROP TABLE musicas;', [], null, self.SQLError);
		tx.executeSql('CREATE TABLE IF NOT EXISTS musicas (audioid, name, artist, band, album, mood_velocity NUMERIC, mood_happy NUMERIC, path, playlist, type, date_start NUMERIC, date_end NUMERIC, hour_start NUMERIC, hour_end NUMERIC, weekdays, min_interval)',[],null, self.SQLError);
...
MusicManager.prototype.nextMusic = function(callback){
	...
	this.db.transaction(function (tx) {
		...
		tx.executeSql('SELECT * FROM musicas', [], function (tx, results) {
			...
			//PRIMEIRA PASSAGEM
			var attemps = 0;
			tx.executeSql('SELECT * FROM musicas '+createWhere(attemps)+' ORDER BY audioid', [], function (tx, results) {
				var ret = [];
				var len = results.rows.length, i;
...
MusicManager.prototype.createWhere = function ( wheres , turns ){
	turns = turns || 0;
}
...
MusicManager.prototype.whereMonthday = function( music_init, music_end ){ // Where para dia do mês
