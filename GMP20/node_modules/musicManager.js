...
MusicManager.prototype.init = function(player_opts, server_options, db){
	...
	this.selectedPoint = { x:50 + (Math.random()*2-4), y:50 + (Math.random()*2-4) };
	self.spots = [];
	self.spotLine = [];
	self.wasSpot = true;
	self.testCicle = 0;
...
MusicManager.prototype.verifyList = function( list , hash, fromDir ){
	...
	if(ret.notfounded.length>0){
		...
		console.log(" - - - - - - - - - - - - - - - - - - ");
		console.log(self.hashUpdate+" - "+hash);
		console.log(" - - - - - - - - - - - - - - - - - - ");
	...
	this.db.transaction(function (tx) {
		tx.executeSql('SELECT audioid, name, mood_velocity, mood_happy FROM musicas', [], function (tx, results) {
			...
			for (i = 0; i < len; i++) {
				...
				var dist_x = (self.selectedPoint.x-mus.mood_velocity+(Math.random()*2-4));
				var dist_y = (self.selectedPoint.y-mus.mood_happy +(Math.random()*2-4));
